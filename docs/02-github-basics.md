...
### 3. Spec Kit の初期化 — Git Flow で体験する SDD セットアップ（10分）

> 💡 **ここからが SDD 体験の第一歩です！**
> Spec Kit の初期化を Git Flow の中で実践し、「環境変更もブランチ→PR→Merge」の流れを身に付けます。
...
> **何が起きるか**: `.specify/` ディレクトリが作成され、SDD ワークフローに必要な
> テンプレート（spec-template、plan-template、tasks-template）とコマンドがセットアップされます。

```
期待される出力:
  .specify/
  ├── memory/           ← プロジェクトの記憶（constitution 等）
  ├── templates/        ← 仕様書・計画・タスクのテンプレート
  └── extensions/       ← 拡張機能
```

#### Step 3: 初期化結果をコミットして PR を作成
...
GitHub で PR を作成し、チームメンバーにレビューしてもらいましょう。

> 💡 **ポイント**: ツールの初期化もコードと同じ Git Flow で管理する。
> これにより「誰が」「いつ」「何のために」変更したかが Pull Request に記録され、
> チーム全体で共有できます。
