...
- GitHub Issue に Copilot Cloud Agent をアサインし、自律的に PR が作成される体験をする
...
> 人間は「何を作るか」を決め、Cloud Agent が「どう作るか」を実行する。
> このループが GitHub Platform 上で完結するため、トレーサビリティが保証される。

#### Copilot Cloud Agent とは
...
### 2. デモ: Cloud Agent の動作確認（10分）

> 📺 **講師デモ**: 事前に準備した Issue → Cloud Agent → PR の3段階を見せる
>
> ⚠️ **Cloud Agent が参加者環境で利用できない場合**:
> このセクションは講師デモのみとなります。参加者は Step 3 の「完了した PR の確認」で
> Cloud Agent が生成した成果物を確認する形で体験します。

#### 事前準備された3つの状態

| 状態 | Issue | Cloud Agent | PR |
|------|-------|-------------|-----|
...
2. Cloud Agent が Issue を受け取り、自動的に作業を開始

> ⏳ Cloud Agent の実行には数分かかります。
...
事前に講師が準備した Cloud Agent の PR を確認:
...
タスク分解（Prompt File）        /speckit.tasks → .specify/tasks.md
   ↓ 手動Issue作成                   ↓ 自動生成・保存
Issue → Cloud Agent             Issue → Cloud Agent
```
...
- **事前準備が最重要**: Cloud Agent による完了済み PR を必ず1つ以上用意しておく
